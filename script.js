...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateUI = () => {
        ...
        // The progress bar logic and display can remain the same to guide the user to the "best" bundle deal
        ...
        // Immediately apply the discount if at least one item is selected
        ...
        if (numSelectedItems >= MIN_ITEMS_FOR_DISCOUNT) {
            ...
            const itemsNeeded = MIN_ITEMS_FOR_DISCOUNT - numSelectedItems;
            bundleStatus.textContent = `Add ${itemsNeeded} more product${itemsNeeded > 1 ? 's' : ''} for a better bundle deal!`;
            ctaButton.innerHTML = `Add ${totalItems} items to Cart ${ctaIconSVG}`;
        } else {
    ...
    ctaButton.addEventListener('click', () => {
        // The condition for adding to cart can be changed here if needed,
        // but it's often a good practice to still require a minimum for a "bundle"
